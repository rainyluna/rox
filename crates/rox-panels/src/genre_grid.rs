...
use gpui::{
    ...
    FontWeight, KeyDownEvent, Modifiers, MouseButton, MouseDownEvent, MouseUpEvent, ObjectFit,
    Pixels, ScrollStrategy, ScrollWheelEvent, SharedString, Size, Subscription, WeakEntity, Window,
    canvas, div, img, linear_color_stop, linear_gradient, prelude::*, px, size, svg,
};
...
use crate::group_head::{MOSAIC, TileFace};
use crate::panel::{
...
impl GenreGridPanel {
    pub fn new(
        ...
        let _type_ahead_blur = window.on_focus_out(&focus, cx, move |_, _, cx| {
            panel
                .update(cx, |this: &mut GenreGridPanel, cx| {
    ...
    fn sync_picks(&mut self, cx: &App) {
        ...
        // still outlines the tile it now belongs to.
        self.selected = self
            .cells
            .iter()
            .enumerate()
    ...
    fn type_to(&mut self, text: String, cx: &mut Context<Self>) {
        ...
        let hit = (0..len).map(|off| (start + off) % len).find(|&ix| {
            self.cells
                .get(ix)
                .is_some_and(|cell| panel::type_ahead_hit(&cell.lower, &needle))
        });
        if let Some(ix) = hit {
            self.selected = HashSet::from([ix]);
            self.anchor = Some(ix);
    ...
    fn type_step(&mut self, back: bool, cx: &mut Context<Self>) {
        ...
        let needle = self.type_ahead.to_lowercase();
        let anchor = self.selected.iter().copied().min().or(self.anchor);
        let hit = panel::type_ahead_scan(len, anchor, back).find(|&ix| {
            self.cells
                .get(ix)
                .is_some_and(|cell| panel::type_ahead_hit(&cell.lower, &needle))
        });
