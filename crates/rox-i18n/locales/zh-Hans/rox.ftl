...
bookmark-name = 名称
bookmark-name-placeholder = 可选，留空则显示时间
bookmark-position = 位于 { $time }
bookmark-color = 颜色
bookmark-color-accent = 主题强调色
bookmark-color-custom = 自定义...
bookmark-color-red = 红色
bookmark-color-orange = 橙色
bookmark-color-yellow = 黄色
bookmark-color-green = 绿色
bookmark-color-teal = 青色
bookmark-color-blue = 蓝色
bookmark-color-purple = 紫色
bookmark-color-pink = 粉色
bookmark-save = 保存
bookmark-hint-before = 按
bookmark-hint-key = Enter
bookmark-hint-after = 保存
bookmark-menu-play = 从此处播放
bookmark-menu-rename = 重命名...
bookmark-menu-color = 颜色
bookmark-menu-move = 移到当前播放位置
bookmark-menu-remove = 删除
...
playback-item-ab-repeat = A-B 循环
playback-item-favourite = 收藏
playback-item-rating = 评分
playback-sleep-15 = 15 分钟后停止
playback-sleep-30 = 30 分钟后停止
playback-sleep-60 = 一小时后停止
playback-sleep-90 = 90 分钟后停止
playback-sleep-off = 不设睡眠定时
playback-sleep-off-remaining = 取消睡眠定时（剩余 { $minutes } 分钟）
playback-sleep-tip = 睡眠定时
playback-sleep-tip-remaining = 睡眠定时，剩余 { $minutes } 分钟
menubar-button-tasks = 任务按钮
menubar-button-sleep = 睡眠定时按钮
menubar-button-rescan = 重新扫描按钮
...
library-group-search-results = 对搜索结果分组
    .description = 搜索时保留当前分组标题；关闭后以平铺列表显示匹配的曲目
...
grid-menu-scroll = 滚动
grid-menu-sort = 排序
grid-sort-artist = 艺术家
grid-sort-album = 专辑
grid-sort-year = 年份
grid-sort-added = 最近添加
grid-sort-plays = 播放最多
grid-letter-rail = 字母索引
    .description = 在墙的边缘显示首字母；点击跳到该字母的第一张专辑
...
settings-appearance-milkdrop-beat-sensitivity = 节拍灵敏度
    .description = projectM 多容易把某个瞬间当成节拍。安静的混音要调得比响亮的高
settings-appearance-milkdrop-color = 颜色
    .description = 预设保留文件本身的颜色。主题在浅色主题下把它们翻转成浅色。调色板用主题的背景色和强调色来上色。封面用正在播放的封面颜色来上色，不管歌曲主题开没开
settings-appearance-milkdrop-duration = 预设时长
    .description = 一个预设保持多久才切到下一个
settings-appearance-milkdrop-enabled = Milkdrop 背景
    .description = 播放音乐时在整个应用后面运行 Milkdrop 视觉效果
settings-appearance-milkdrop-error-title = Milkdrop 背景没能启动
settings-appearance-milkdrop-fade-duration = 淡出时长
    .description = 淡出所需的时间：暂停或停止时淡出，播放时淡入
settings-appearance-milkdrop-flip-horizontal = 水平翻转
    .description = 把画面左右镜像
settings-appearance-milkdrop-flip-vertical = 垂直翻转
    .description = 把画面上下镜像
settings-appearance-milkdrop-fps = 帧率
    .description = 渲染线程的目标值。设成 30 能把每帧回读的开销减半，模糊之下的背景很少需要更多
settings-appearance-milkdrop-hard-cuts = 硬切
    .description = 足够响的节拍可以直接切到下一个预设
settings-appearance-milkdrop-idle = 空闲时
    .description = 播放暂停或停止时视觉效果的处理方式。保持会把最后一帧留在应用背后。淡出会退到封面背景，播放时再回来
settings-appearance-milkdrop-locked = 锁定预设
    .description = 停留在当前预设上，换曲和重启都不变
settings-appearance-milkdrop-preset = 预设
    .description = 背景显示哪个预设。不播放时也能选，重启后保留
settings-appearance-milkdrop-rotation = 轮换范围
    .description = 下一个、上一个和定时切换会在哪些预设之间轮换
settings-appearance-milkdrop-scale = 渲染比例
    .description = 视觉效果按窗口分辨率的多大比例渲染。越低越省；这么柔和的背景很少需要更高
settings-appearance-milkdrop-strength = 强度
    .description = 视觉效果盖过模糊封面的程度
...
settings-application-menu-entry = 应用菜单项
    .description = 为这个 AppImage 添加启动器项和文件关联，文件挪到哪里就指向哪里
...
settings-application-section-desktop = 桌面
...
settings-integrations-ffmpeg-binary = FFmpeg 可执行文件
    .description = 用哪个 ffmpeg 做转换；留空就用 PATH 上的那个，或者 rox 数据文件夹里的构建
...
settings-integrations-ffmpeg-missing-note-flatpak = “转换”还藏着。把静态构建的 ffmpeg 放进 rox 的数据文件夹；系统里装的那个在沙箱内跑不起来。
settings-integrations-ffmpeg-missing-title = 没找到能用的 ffmpeg
settings-integrations-ffmpeg-ok-note = ffmpeg 能用。“转换”已可用。
settings-integrations-ffmpeg-reveal = 显示数据文件夹
...
settings-integrations-librefm-intro = 连接你的 Libre.fm 账号：在浏览器里给 rox 授权，播过的曲目越过阈值后就会 scrobble 过去
settings-integrations-listenbrainz-get-token = 获取令牌
settings-integrations-listenbrainz-intro = 粘贴来自 listenbrainz.org/settings 的用户令牌。曲目越过 scrobble 阈值后就会发送收听记录。
settings-integrations-listenbrainz-token-placeholder = 令牌
settings-integrations-listenbrainz-token-row = 用户令牌
settings-integrations-love-favourites = 收藏同步为喜爱
    .description = 把红心同步到 Last.fm 作为喜爱的曲目；取消红心那边也会取消
settings-integrations-scrobble-connect = 连接
settings-integrations-scrobble-disconnect = 断开连接
settings-integrations-scrobble-finish-connecting = 完成连接
settings-integrations-scrobble-reconnect = 重新连接
settings-integrations-scrobble-status-checking = 正在检查令牌
settings-integrations-scrobble-status-confirming = 正在确认…
settings-integrations-scrobble-status-connected = 已连接为 { $username }
settings-integrations-scrobble-status-failed = 连接失败：{ $error }
settings-integrations-scrobble-status-invalid = { $service } 拒绝了该令牌
settings-integrations-scrobble-status-not-connected = 未连接
settings-integrations-scrobble-status-rejected = { $service } 拒绝了这个会话，已经丢弃。重新连接才能继续 scrobble
settings-integrations-scrobble-status-requesting = 正在请求令牌…
settings-integrations-scrobble-status-waiting = 在浏览器里给 rox 授权，然后完成连接
settings-integrations-scrobble-threshold = Scrobble 阈值
    .description = 一首曲目播到多少才 scrobble，对每个已连接的服务都一样；进度条和波形可以标出这个位置
settings-integrations-scrobble-tracks = Scrobble 曲目
    .description = 播过的曲目越过阈值后发给每个已连接的服务
settings-integrations-scrobble-working = 处理中…
...
settings-library-portal-hint = 把 /path/to 换成文件夹的真实位置。
settings-library-portal-note = 这样能用，但对曲库来说很慢，而且 rox 无法监视它的变动。把文件夹的真实位置授权给 rox，然后移除并重新添加它：
settings-library-portal-title = rox 通过文件门户访问这个文件夹
...
settings-providers-acoustid = AcoustID
    .description = 取音频指纹，再问 acoustid.org 这是哪一版录音；元数据比对里的按声音识别按钮，连标签错得没法搜的文件也能找到发行版
settings-providers-acoustid-key = AcoustID 密钥
settings-providers-acoustid-key-hint = 可选；本构建自带密钥
settings-providers-acoustid-key-placeholder = 应用密钥
...
settings-storage-looks-layouts = 外观和布局
    .description = 应用当前使用的这套外观（workspace.json），连同你保存的工作区和导出的着色器文件。很小，而且每一个字节都是你亲手设的
...
welcome-menu-add = 添加到应用菜单
welcome-menu-failed = 无法添加菜单项
welcome-menu-not-now = 暂不
welcome-menu-note = AppImage 不会自己注册。这会添加一个启动器项和音频文件关联；主目录之外什么都不会改动。
welcome-menu-title = 把 rox 添加到应用菜单？
...
keymap-mod-win = Win
keymap-new-window = 新建窗口
    .description = 用保存的布局再开一个工作窗口
keymap-next-track = 下一首
    .description = 跳到队列里的下一首曲目
keymap-open-about = 关于
    .description = 显示版本和致谢
...
keymap-open-console = 控制台
    .description = 打开日志控制台
keymap-open-equalizer = 均衡器
    .description = 打开均衡器窗口
keymap-open-preset-picker = 背景预设
    .description = 打开背景的 Milkdrop 预设选择器
...
keymap-ab-repeat = A-B 循环
    .description = 标记 A，再标记 B，再按清除
keymap-ab-clear = Clear A-B Section
    .description = Drop the A-B section and any half-marked step with it
keymap-bookmark = 添加书签
    .description = 在当前播放位置添加书签
keymap-bookmark-named = 添加带名称的书签
    .description = 在当前播放位置添加书签并命名
keymap-prev-bookmark = 上一个书签
...
tags-editor-replace = 替换
tags-editor-replace-apply = 应用
tags-editor-replace-find = 查找
tags-editor-replace-help = 每处匹配都会改写。开启正则后，$1 插入第一个分组。结果会去掉两端空白。
tags-editor-replace-ignore-case = 忽略大小写
tags-editor-replace-match-count = { $total } 个里更改 { $hits } 个
tags-editor-replace-regex = 正则
tags-editor-replace-with = 替换为
...
tags-matcher-identify = 按声音识别
tags-matcher-identify-no-duration = 没有曲目时长可用于识别
...
tags-rename-pattern-help = { $placeholders }；/ 从音乐库根目录建文件夹，没有 / 则原地重命名，扩展名沿用原文件的
...
waveform-loudness = 响度带
    .description = 把 RMS 电平画成峰值包络内侧一条更扁的带子
waveform-gradient-mode = 取色来源
    .description = 包络和带子从哪里取色：强调色、主题的渐变、封面，或者一对自定义颜色
waveform-scrobble-marker = Scrobble 标记
    .description = 一条细线，标出曲目算作已 scrobble 到 Last.fm 的位置
waveform-bookmarks = 书签
...
metadata-cover-opacity = 封面不透明度
    .description = 封面在字段后面显示得多强
metadata-display = 显示
...
metadata-field-folder = 文件夹
metadata-field-gain-album = 专辑增益
metadata-field-gain-track = 曲目增益
metadata-field-last-played = 上次播放
metadata-field-lastfm-listeners = 全球听众
metadata-field-lastfm-loved = Last.fm 喜爱
metadata-field-lastfm-plays = 全球播放
metadata-field-lastfm-tags = Last.fm 标签
metadata-field-lastfm-user-plays = 你的 Scrobble 数
metadata-field-lyrics = 歌词
metadata-field-mb-country = 发行国家
metadata-field-mb-date = 发行日期
metadata-field-mb-first-release = 首次发行
metadata-field-mb-isrc = ISRC
metadata-field-mb-label = 厂牌
metadata-field-missing = 缺失
metadata-field-rank = 曲库排名
metadata-field-recent-plays = 播放次数 (30 天)
metadata-field-sample-rate = 采样率
metadata-field-similar = 相似曲目
metadata-field-size = 大小
metadata-field-title-sort = 排序标题
metadata-field-track = 曲目
metadata-fields = 字段
    .description = 面板列出哪些字段；没有值的字段显示“无”，排序名只在不同的时候显示
...
metadata-cover-embedded = 内嵌，{ $px }
metadata-cover-folder = { $file }，{ $px }
metadata-hide-empty = 隐藏空字段
    .description = 省略没有值的已启用字段，而不是显示为“无”
metadata-loved-no = 否
metadata-loved-yes = 是
metadata-lyrics-plain = 纯文本
metadata-lyrics-synced = 带时间轴
metadata-missing-none = 无
metadata-rank-value = 第 { $rank } 名
metadata-tags-scope-album = (专辑标签)
metadata-tags-scope-artist = (艺术家标签)
metadata-track-of = 第 { $track } 首，共 { $total } 首
metadata-value-none = 无
metadata-source = 来源
...
folder-tree-collapse-branch = 折叠分支
...
folder-tree-expand-branch = 展开分支
...
art-label-position = 标签位置
    .description = 专辑标注的位置：顶部、封面下方、底部或隐藏
art-letter-rail = 字母索引
    .description = 在架子边缘显示艺术家首字母；点击跳到该字母的第一张专辑
art-layout-section = 布局
art-perspective = 透视
    .description = 按下面的角度转动两侧封面；关闭时保持扁平的方形，也只有这时封面圆角才起作用
art-recede = 纵深亮度
    .description = 最里面那张封面有多亮；它和中间之间的封面平均分摊这段差值
art-spacing = 封面间距
    .description = 中间封面到左右第一张封面的距离；超过一半就会让开中间封面，给它留出空隙
art-stride = 堆叠间距
    .description = 第一张之后各封面之间的距离；也决定拖动一张封面要走多远
art-visible = 显示数量
    .description = 中间封面左右各排几张；最外一张会淡出而不是直接消失
art-tilt = 转动角度
    .description = 两侧封面向后转开的幅度
art-reflections = 倒影
...
## Biography panel
biography-active-between = 活动于 { $from } 至 { $to }
biography-active-since = 自 { $from } 起活动
biography-background = 背景
    .description = 文字后面的艺术家画面，压暗并向底部淡出
biography-background-opacity = 背景不透明度
    .description = 文字后面的画面显示得多强
biography-cycle = 轮换头图
    .description = 在头图中交叉淡入淡出地轮换艺术家的横幅和画面，悬停时出现的箭头可以手动切换
biography-cycle-interval = 轮换间隔
    .description = 每张头图停留多久，之后下一张淡入
biography-fill-width = 填满宽度
    .description = 让高的头图铺满整个宽度，而不是限宽居中
biography-flag = 国旗
    .description = 用国旗而不是两位代码显示国家
biography-from-lastfm = 来自 Last.fm
biography-header-banner = 横幅条
    .description = TheAudioDB 为艺术家保存的宽幅标志条，放在头图中
biography-header-blur = 模糊填充适配
    .description = 关闭比例时，把整张图放进头图条，底下铺一层模糊的自身，而不是裁切
biography-header-fanart = 画面
    .description = TheAudioDB 的艺术家 16:9 画面，放在头图中
biography-header-height = 头图高度
    .description = 头图条的高度 (px)；保持比例时则是其上限
biography-header-image = 头图
    .description = 顶部横跨的宽幅艺术家横幅；没有横幅时用肖像
biography-header-overlay = 标题叠在头图上
    .description = 把名字、来源和活动年份叠在头图底部，而不是放在下面
...
biography-name-source = 名字来源
    .description = 用哪个标签作为艺术家：曲目自身的艺术家，或者合辑和客串较多的专辑用专辑艺术家
...
biography-profile = 来源与年份
    .description = 来自 TheAudioDB 的国家代码和活动年份
biography-refresh = 刷新
biography-seconds = { $count } 秒
biography-similar-artists = 相似艺术家
    .description = 按收听数据得出的相关艺术家，以标签显示：点击转到该艺术家的页面，曲库里有的带一个搜索图标，点它在搜索中选中
...
biography-title = 简介
biography-top-tracks = 热门曲目
    .description = Last.fm 上播放最多的曲目，点击播放曲库里有的，加号则加入队列
biography-top-tracks-count = 曲目数量
    .description = 列出多少首热门曲目
biography-top-tracks-heading = 热门曲目
...
playback-ab-clear = 清除 A-B
playback-ab-set-a = 设置 A
playback-ab-set-b = 设置 B
playback-ab-tip-a = A 在 { $a }。再按一次标记 B
playback-ab-tip-looping = 正在循环 { $a } 到 { $b }
playback-ab-tip-off = 标记要循环的段落起点
playback-hold-order = { $tip }。按住可以选顺序
playback-loop-off = 循环已关
playback-loop-queue = 循环整个队列
playback-loop-track = 循环这首曲目
playback-menu-ab-repeat = A-B 循环按钮
...
milkdrop-presets = 预设
milkdrop-tuning = 调节
milkdrop-roots = 预设文件夹
milkdrop-add-root = 添加文件夹
milkdrop-remove-root = 移除
milkdrop-folder = rox 查找的位置
milkdrop-filter-presets = 筛选预设
milkdrop-preset-count = { $total } 个预设中的 { $count } 个
milkdrop-locked = 锁定预设
    .description = 就停在这个：不按时间切换，也不随节拍切换
milkdrop-duration = 预设时长
    .description = 一个预设保持多久，下一个才接上
milkdrop-beat-sensitivity = 节拍灵敏度
    .description = projectM 多容易把某个瞬间当成节拍。安静的混音要调得比响亮的高
milkdrop-hard-cuts = 硬切
    .description = 足够响的节拍可以直接切到下一个预设
milkdrop-fps = 帧率
    .description = 渲染线程的目标值。设成 30 能把每帧回读的开销减半
milkdrop-scale = 渲染缩放
    .description = 以低于面板的尺寸渲染再放大。损失清晰度，同比例省下回读开销
milkdrop-tint = 专辑配色
    .description = 把画面的色相转向正在播放曲目的封面。设为零则保留预设本来的颜色
milkdrop-fade-duration = 淡出时长
milkdrop-idle = 空闲时
    .description = 播放暂停或停止时视觉效果的处理方式。保持会把最后一帧留在屏幕上。淡出会退到面板背景，播放时再回来
milkdrop-idle-hold = 保持
milkdrop-idle-fade = 淡出
milkdrop-run-focused = 聚焦时继续运行
    .description = 面板拥有焦点时，无论是否在播放，视觉效果都会继续运行。关闭后，即使面板处于焦点，保持的画面也会静止
milkdrop-show-name = 显示预设名称
    .description = 预设切换时在角落显示其名称几秒钟
milkdrop-next = 下一个预设
milkdrop-previous = 上一个预设
milkdrop-random = 随机预设
milkdrop-open-folder = 打开预设文件夹
milkdrop-rescan = 重新扫描
milkdrop-rotation = 轮换范围
    .description = 下一个、上一个和定时切换会在哪些预设之间轮换
milkdrop-rotation-all = 全部预设
milkdrop-empty = 没有找到预设。
    .description = 把预设包放进下面的文件夹，然后点重新扫描。这三个最值得装，都在 GitHub 的 projectM-visualizer 下。
milkdrop-packs-hint = 把预设包放进这里。值得装的三个是 presets-cream-of-the-crop、presets-milkdrop-original 和 presets-milkdrop-texture-pack，都在 GitHub 的 projectM-visualizer 下。
milkdrop-no-gl = Milkdrop 没能启动。
milkdrop-still-starting = Milkdrop 仍在启动。
milkdrop-still-starting-detail = 正在等待图形驱动提供 OpenGL 上下文。
milkdrop-no-frames = Milkdrop 正在运行，但还没有收到任何一帧。
milkdrop-no-frames-detail = 在 { $renderer }（{ $version }）上渲染。详情见控制台。
milkdrop-favorite = 收藏预设
milkdrop-favorite-short = 收藏
milkdrop-favorited = 已收藏
milkdrop-favorites-only = 仅收藏
milkdrop-rotation-favorites = 收藏 ({ $count })
milkdrop-no-favorites = 还没有加星的预设，所以这里会随机整个库。在列表、面板菜单或“正在显示”一行给预设加星。
milkdrop-reveal = 在文件夹中显示预设
milkdrop-reveal-short = 显示位置
milkdrop-current = 正在显示
milkdrop-no-preset = 还没有加载任何预设
milkdrop-color = 颜色
    .description = 预设保留文件本身的颜色。主题在浅色主题下把它们翻转成浅色。调色板用主题的背景色和强调色来上色，并不再套用专辑色。封面用正在播放的封面颜色来上色，不管歌曲主题开没开
milkdrop-color-preset = 预设
milkdrop-color-theme = 主题
milkdrop-color-palette = 调色板
milkdrop-color-cover = 封面
milkdrop-name-always = 始终显示名称
    .description = 始终显示名称，而不只是几秒钟
milkdrop-flip-horizontal = 水平翻转
    .description = 把画面左右镜像
milkdrop-flip-vertical = 垂直翻转
    .description = 把画面上下镜像
milkdrop-choose = 选择预设
milkdrop-picker-title = 选择预设
milkdrop-picker-backdrop = 背景
milkdrop-picker-host = 正在为 { $host } 选择
milkdrop-view-tree = 树形
milkdrop-view-grid = 网格
milkdrop-nested = 按文件夹
milkdrop-locate-current = 跳到当前
milkdrop-load-preset = 加载预设
milkdrop-unfavorite = 取消收藏
