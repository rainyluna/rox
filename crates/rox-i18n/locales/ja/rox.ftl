...
bookmark-edit-title = ブックマークを編集
bookmark-window-title = rox - { $verb }
bookmark-name = 名前
bookmark-name-placeholder = 任意。空欄なら時刻が表示されます
bookmark-position = 位置 { $time }
bookmark-color = 色
bookmark-color-accent = テーマのアクセント
bookmark-color-custom = カスタム...
bookmark-color-red = 赤
bookmark-color-orange = オレンジ
bookmark-color-yellow = 黄
bookmark-color-green = 緑
bookmark-color-teal = ティール
bookmark-color-blue = 青
bookmark-color-purple = 紫
bookmark-color-pink = ピンク
bookmark-save = 保存
bookmark-hint-before = 保存は
bookmark-hint-key = Enter
bookmark-hint-after = キー
bookmark-menu-play = ここから再生
bookmark-menu-rename = 名前を変更...
bookmark-menu-color = 色
bookmark-menu-move = 再生位置へ移動
bookmark-menu-remove = 削除
...
playback-item-ab-repeat = A-Bリピート
playback-item-favourite = お気に入り
playback-item-rating = レーティング
playback-sleep-15 = 15分後に停止
playback-sleep-30 = 30分後に停止
playback-sleep-60 = 1時間後に停止
playback-sleep-90 = 90分後に停止
playback-sleep-off = スリープタイマーなし
playback-sleep-off-remaining = スリープタイマーを解除 (残り { $minutes } 分)
playback-sleep-tip = スリープタイマー
playback-sleep-tip-remaining = スリープタイマー、残り { $minutes } 分
menubar-button-tasks = タスクボタン
menubar-button-sleep = スリープタイマーボタン
menubar-button-rescan = 再スキャンボタン
...
library-group-search-results = 検索結果をグループ化
    .description = 検索中も現在のグループ見出しを維持する。オフにすると一致したトラックをフラットな一覧で表示する
...
settings-appearance-milkdrop-beat-sensitivity = ビート感度
    .description = projectM がどれくらい簡単にビートとみなすか。静かなミックスほど高めにする
settings-appearance-milkdrop-color = 色
    .description = プリセットはファイル本来の色のまま。テーマはライトテーマのとき明るさを反転する。パレットはテーマの背景色とアクセント色で塗る。ジャケットは再生中のジャケットの色で塗る。曲ごとのテーマの設定に関わらず
settings-appearance-milkdrop-duration = プリセットの長さ
    .description = 次のプリセットに切り替わるまでの時間
settings-appearance-milkdrop-enabled = Milkdrop 背景
    .description = 再生中、アプリ全体の背後で Milkdrop のビジュアルを動かす
settings-appearance-milkdrop-error-title = Milkdrop 背景を起動できなかった
settings-appearance-milkdrop-fade-duration = フェード時間
    .description = フェードにかける時間。一時停止や停止で消え、再生で戻る
settings-appearance-milkdrop-flip-horizontal = 左右反転
    .description = ビジュアルを左右に反転する
settings-appearance-milkdrop-flip-vertical = 上下反転
    .description = ビジュアルを上下に反転する
settings-appearance-milkdrop-fps = フレームレート
    .description = 描画スレッドが目標にする値。30 にすると 1 フレームの読み戻しの負担が半分になり、ぼかしの下の背景にはそれで十分なことが多い
settings-appearance-milkdrop-hard-cuts = ハードカット
    .description = 十分に強いビートで次のプリセットへ即座に切り替える
settings-appearance-milkdrop-idle = 停止中
    .description = 再生が一時停止または停止しているあいだのビジュアルの扱い。保持は最後のフレームをアプリの背後に残す。フェードはカバーの背景へ消え、再生で戻る
settings-appearance-milkdrop-locked = プリセットを固定
    .description = 曲が変わっても再起動しても、今のプリセットにとどまる
settings-appearance-milkdrop-preset = プリセット
    .description = 背景に表示するプリセット。再生していなくても選べ、再起動後も残る
settings-appearance-milkdrop-rotation = ローテーション
    .description = 次へ、前へ、時間切り替えがめぐるプリセットの範囲
settings-appearance-milkdrop-scale = レンダリング倍率
    .description = ウィンドウ解像度の何割でビジュアルを描くか。低いほど軽い。これだけ柔らかい背景なら高くする必要はまずない
settings-appearance-milkdrop-strength = 強さ
    .description = ぼかしたジャケットの上にビジュアルをどこまで乗せるか
...
settings-appearance-section-milkdrop = Milkdrop 背景
...
settings-application-menu-entry = アプリメニューの項目
    .description = この AppImage のランチャー項目とファイルの関連付け。ファイルを移動しても、その場所を指し続けます
...
settings-application-section-desktop = デスクトップ
...
settings-integrations-ffmpeg-binary = FFmpeg の実行ファイル
    .description = 変換に使う ffmpeg。空なら PATH 上のものを使うか、rox のデータフォルダーにあるビルドを使う
...
settings-integrations-ffmpeg-missing-note-flatpak = 「変換」は隠れたままです。静的ビルドの ffmpeg を rox のデータフォルダーに置いてください。システムに入っているものはサンドボックス内では動きません。
settings-integrations-ffmpeg-missing-title = 動く ffmpeg が見つかりません
settings-integrations-ffmpeg-ok-note = ffmpeg は動きます。「変換」が使えます。
settings-integrations-ffmpeg-reveal = データフォルダーを開く
...
settings-integrations-librefm-intro = Libre.fm アカウントに接続します。ブラウザーで rox を承認すると、しきい値を超えた再生曲がスクロブルされます
settings-integrations-listenbrainz-get-token = トークンを取得
settings-integrations-listenbrainz-intro = listenbrainz.org/settings のユーザートークンを貼り付けてください。再生がスクロブルのしきい値を超えると送信します。
settings-integrations-listenbrainz-token-placeholder = トークン
settings-integrations-listenbrainz-token-row = ユーザートークン
settings-integrations-love-favourites = お気に入りを Love に反映
    .description = ハートを Last.fm の Loved Tracks に反映する。ハートを外すと向こうでも外れる
settings-integrations-scrobble-connect = 接続
settings-integrations-scrobble-disconnect = 切断
settings-integrations-scrobble-finish-connecting = 接続を完了する
settings-integrations-scrobble-reconnect = 再接続
settings-integrations-scrobble-status-checking = トークンを確認中
settings-integrations-scrobble-status-confirming = 確認中...
settings-integrations-scrobble-status-connected = { $username } として接続中
settings-integrations-scrobble-status-failed = 接続に失敗: { $error }
settings-integrations-scrobble-status-invalid = { $service } はこのトークンを拒否しました
settings-integrations-scrobble-status-not-connected = 未接続
settings-integrations-scrobble-status-rejected = { $service } がセッションを拒否したため破棄しました。スクロブルを続けるには接続し直してください
settings-integrations-scrobble-status-requesting = トークンを要求中...
settings-integrations-scrobble-status-waiting = ブラウザーで rox を承認してから、接続を完了してください
settings-integrations-scrobble-threshold = スクロブルのしきい値
    .description = 曲のどれくらいを再生したらスクロブルするか。接続中のどのサービスにも共通。シークバーと波形に印を出せる
settings-integrations-scrobble-tracks = 曲をスクロブル
    .description = しきい値を超えた曲を接続中のすべてのサービスに送る
settings-integrations-scrobble-working = 処理中...
...
settings-library-portal-hint = /path/to をフォルダーの実際の場所に置き換えてください。
settings-library-portal-note = 動きはしますが、ライブラリには遅く、rox は変更を監視できません。フォルダーの実際の場所を rox に許可してから、いったん削除して追加し直してください:
settings-library-portal-title = rox はこのフォルダーにファイルポータル経由で届いています
...
settings-providers-acoustid = AcoustID
    .description = 音声のフィンガープリントを取り、どの録音かを acoustid.org に問い合わせる。メタデータ比較の「音で特定」ボタンなら、タグが当てにならないファイルでもリリースが見つかる
settings-providers-acoustid-key = AcoustID キー
settings-providers-acoustid-key-hint = 任意。このビルドは自前のキーを持っている
settings-providers-acoustid-key-placeholder = アプリケーションキー
...
settings-storage-looks-layouts = 見た目とレイアウト
    .description = 今アプリが使っている見た目 (workspace.json) と、保存したワークスペース、書き出したシェーダーファイル。小さく、その一バイト一バイトが自分で作ったもの
...
health-desc-writable = rox が読めてもタグを書き戻せない形式のトラック。まれな形式の断片化した MP4 も書き込みを拒みますが、ここには数えていません。
...
welcome-menu-add = アプリメニューに追加
welcome-menu-failed = メニュー項目を追加できませんでした
welcome-menu-not-now = 今はしない
welcome-menu-note = AppImage は自分で登録されません。これはランチャー項目と音声ファイルの関連付けを追加します。ホームフォルダーの外は何も変わりません。
welcome-menu-title = rox をアプリメニューに追加しますか？
...
eq-gain-label = ゲイン
eq-heading = イコライザー
eq-help-text = バンドをドラッグすると動き、上でスクロールすると幅が変わります。処理はサウンドカードに送るバッファより手前にありますが、このウィンドウが開いている間は、動かしてからスピーカーに届くまで 120 ミリ秒ほどです。
eq-hint-off = クリックでオフ
eq-hint-on = クリックでオン
...
keymap-mod-win = Win
keymap-new-window = 新しいウィンドウ
    .description = 保存したレイアウトで作業ウィンドウをもう一つ開く
keymap-next-track = 次の曲
    .description = キューの次の曲へ進む
keymap-open-about = rox について
    .description = バージョンとクレジットを表示する
...
keymap-open-console = コンソール
    .description = ログコンソールを開く
keymap-open-equalizer = イコライザー
    .description = イコライザーのウィンドウを開く
keymap-open-preset-picker = 背景のプリセット
    .description = 背景の Milkdrop プリセット選択を開く
...
keymap-ab-repeat = A-Bリピート
    .description = Aを指定し、次にB、もう一度で解除
keymap-ab-clear = Clear A-B Section
    .description = Drop the A-B section and any half-marked step with it
keymap-bookmark = ブックマークを追加
    .description = 再生位置にブックマークを置く
keymap-bookmark-named = 名前付きブックマークを追加
    .description = 再生位置にブックマークを置いて名前を付ける
keymap-prev-bookmark = 前のブックマーク
...
tags-editor-replace = 置換
tags-editor-replace-apply = 適用
tags-editor-replace-find = 検索
tags-editor-replace-help = 一致した箇所はすべて書き換わる。正規表現では $1 が最初のグループを挿入する。結果は両端の空白を取り除く。
tags-editor-replace-ignore-case = 大文字と小文字を区別しない
tags-editor-replace-match-count = { $total } 件中 { $hits } 件が変わる
tags-editor-replace-regex = 正規表現
tags-editor-replace-with = 置換後
...
tags-matcher-identify = 音で特定
tags-matcher-identify-no-duration = 特定に使える曲の長さがない
...
tags-rename-pattern-help = { $placeholders }。/ はライブラリのルートからフォルダーを作り、/ がなければその場で名前を変え、拡張子はファイルのものが付く
...
lyrics-edit-offset = オフセット
lyrics-edit-offset-unit = 秒
...
pass-tempo-body = 各ファイルから 30 秒の区間を 2 つデコードしてビートを数え、その 2 つが食い違うときはさらに区間を読んで、曲の速さをライブラリに出せるようにします。クリックに合わせて録音された音楽で最もよく働き、測れないものは飛ばします。数値はライブラリのデータベースに入り、ファイルには触れません。
...
waveform-loudness = ラウドネス帯
    .description = RMS レベルをピーク包絡線の内側に平たい帯として描く
waveform-gradient-mode = 色のもと
    .description = 包絡線と帯がどこから色を取るか。アクセント、テーマのグラデーション、カバーアート、またはカスタムの 2 色
waveform-scrobble-marker = スクロブルの印
    .description = 曲が Last.fm にスクロブルされたとみなされる位置の細い線
waveform-bookmarks = ブックマーク
...
metadata-cover-opacity = カバーの不透明度
    .description = 項目の後ろにカバーをどれだけ強く出すか
metadata-display = 表示
...
metadata-field-bookmarks = ブックマーク
...
metadata-field-folder = フォルダ
metadata-field-gain-album = アルバムゲイン
metadata-field-gain-track = トラックゲイン
metadata-field-last-played = 最後に再生
metadata-field-lastfm-listeners = 世界のリスナー
metadata-field-lastfm-loved = Last.fm でお気に入り
metadata-field-lastfm-plays = 世界の再生数
metadata-field-lastfm-tags = Last.fm のタグ
metadata-field-lastfm-user-plays = 自分の Scrobble 数
metadata-field-lyrics = 歌詞
metadata-field-mb-country = 発売国
metadata-field-mb-date = 発売日
metadata-field-mb-first-release = 初出
metadata-field-mb-isrc = ISRC
metadata-field-mb-label = レーベル
metadata-field-missing = 不足
metadata-field-rank = ライブラリ内の順位
metadata-field-recent-plays = 再生数 (30 日)
metadata-field-sample-rate = サンプルレート
metadata-field-similar = 似ている曲
metadata-field-size = サイズ
metadata-field-title-sort = ソート用タイトル
metadata-field-track = トラック
metadata-fields = 項目
    .description = シートに並べる項目。値のない項目は「なし」と出し、読みは異なる場合だけ出す
...
metadata-cover-embedded = 埋め込み、{ $px }
metadata-cover-folder = { $file }、{ $px }
metadata-hide-empty = 空の項目を隠す
    .description = 値のない項目を「なし」と出さずに省く
metadata-loved-no = いいえ
metadata-loved-yes = はい
metadata-lyrics-plain = テキストのみ
metadata-lyrics-synced = 同期あり
metadata-missing-none = なし
metadata-rank-value = { $rank } 位
metadata-tags-scope-album = (アルバムのタグ)
metadata-tags-scope-artist = (アーティストのタグ)
metadata-track-of = { $track } / { $total }
metadata-value-none = なし
metadata-source = ソース
...
art-perspective = パース
    .description = 下の角度で両側のカバーを回す。オフなら平たい正方形のまま。カバーの角丸が効くのはこのときだけ
art-recede = 奥の明るさ
    .description = いちばん奥のカバーをどれだけ明るく置くか。中央との間のカバーはその差を均等に分ける
art-spacing = カバーの間隔
    .description = 中央のカバーから左右一枚目までの距離。半分を超えると中央のカバーから離れ、余白ができる
art-stride = 重なりの間隔
    .description = 一枚目より奥のカバー同士の間隔。ドラッグ一枚ぶんの移動量も決める
art-visible = 表示する枚数
    .description = 中央のカバーの左右に並べる枚数。いちばん外側は消えながら外れる
art-tilt = 回転角度
    .description = 両側のカバーがどれだけ奥へ回るか
art-reflections = 反射
...
## Biography panel
biography-active-between = { $from } 年から { $to } 年まで活動
biography-active-since = { $from } 年から活動中
biography-background = 背景
    .description = 文章の後ろのアーティストのファンアート。暗くして下に向かってフェードする
biography-background-opacity = 背景の不透明度
    .description = 文章の後ろにファンアートをどれだけ強く出すか
biography-cycle = ヘッダー画像を切り替える
    .description = ヘッダーでアーティストのバナーとファンアートをクロスフェードで順に出す。マウスを載せると出る矢印で手動でも切り替えられる
biography-cycle-interval = 切り替え間隔
    .description = 次の画像がフェードインするまで各ヘッダー画像を出しておく時間
biography-fill-width = 幅いっぱいに
    .description = 高いヘッダーを、幅を制限して中央に置かず、全幅に広げる
biography-flag = 国旗
    .description = 国を 2 文字のコードではなく国旗で出す
biography-from-lastfm = Last.fm より
biography-header-banner = バナー
    .description = TheAudioDB が持つアーティストの横長ロゴ帯をヘッダーに出す
biography-header-blur = ぼかし背景で収める
    .description = 比率を保たない場合、画像を切り取らず、ぼかした自身の上に全体を収める
biography-header-fanart = ファンアート
    .description = TheAudioDB のアーティストの 16:9 ファンアートをヘッダーに出す
biography-header-height = ヘッダーの高さ
    .description = ヘッダー帯の高さ (px)。比率を保つ場合はその上限
biography-header-image = ヘッダー画像
    .description = 上部の横長のアーティストバナー。バナーが無ければ写真
biography-header-overlay = ヘッダー上にタイトル
    .description = 名前、出身、活動年を下に置かず、ヘッダー画像の下端に重ねる
...
biography-name-source = 名前の元
    .description = アーティスト名をどのタグから取るか。曲のアーティストか、コンピレーションやゲストの多い作品ならアルバムアーティスト
...
biography-profile = 出身と活動年
    .description = TheAudioDB による国コードと活動年
biography-refresh = 更新
biography-seconds = { $count } 秒
biography-similar-artists = 似たアーティスト
    .description = 聴かれ方の近い関連アーティストをチップで出す。クリックでそのアーティストのページに移り、ライブラリにあるものには検索で選ぶための虫眼鏡が付く
...
biography-top-tracks = 人気曲
    .description = Last.fm で最も再生された曲。ライブラリにある曲はクリックで再生、プラスでキューに追加
biography-top-tracks-count = 曲数
    .description = 人気曲をいくつ並べるか
biography-top-tracks-heading = 人気曲
...
playback-ab-clear = A-Bを解除
playback-ab-set-a = Aを設定
playback-ab-set-b = Bを設定
playback-ab-tip-a = Aは { $a }。もう一度押すとBを設定
playback-ab-tip-looping = { $a } から { $b } を繰り返し中
playback-ab-tip-off = 繰り返す区間の始点を指定します
playback-hold-order = { $tip }。長押しで順序を選択
playback-loop-off = リピート オフ
playback-loop-queue = キューをリピート
playback-loop-track = この曲をリピート
playback-menu-ab-repeat = A-Bリピートボタン
...
milkdrop-presets = プリセット
milkdrop-tuning = 調整
milkdrop-roots = プリセットのフォルダ
milkdrop-add-root = フォルダを追加
milkdrop-remove-root = 削除
milkdrop-folder = rox が探す場所
milkdrop-filter-presets = プリセットを絞り込む
milkdrop-preset-count = { $total } 件中 { $count } 件のプリセット
milkdrop-locked = プリセットを固定
    .description = このまま変えない。時間による切り替えもビートでの切り替えもしない
milkdrop-duration = プリセットの表示時間
    .description = 次に移るまで 1 つのプリセットを表示する長さ
milkdrop-beat-sensitivity = ビート感度
    .description = projectM がどれくらい簡単にビートとみなすか。静かなミックスほど高めにする
milkdrop-hard-cuts = ハードカット
    .description = 十分に強いビートで次のプリセットへ即座に切り替える
milkdrop-fps = フレームレート
    .description = 描画スレッドが目標にする値。30 にすると 1 フレームの読み戻しの負担が半分になる
milkdrop-scale = 描画スケール
    .description = パネルより小さく描画して拡大する。鮮明さと引き換えに、同じ割合だけ読み戻しが軽くなる
milkdrop-tint = アルバムの色
    .description = 映像の色相を再生中の曲のカバーアートへ寄せる。ゼロならプリセット本来の色のまま
milkdrop-fade-duration = フェード時間
milkdrop-idle = 停止中
    .description = 再生が一時停止または停止しているあいだのビジュアルの扱い。保持は最後のフレームを画面に残す。フェードはパネルの背景へ消え、再生で戻る
milkdrop-idle-hold = 保持
milkdrop-idle-fade = フェード
milkdrop-run-focused = フォーカス中は動かし続ける
    .description = パネルにフォーカスがある間は、再生中でなくてもビジュアルを動かし続ける。オフでは、フォーカス中でも保持したフレームは止まったまま
milkdrop-show-name = プリセット名を表示
    .description = プリセットが切り替わったとき、名前を隅に数秒表示する
milkdrop-next = 次のプリセット
milkdrop-previous = 前のプリセット
milkdrop-random = ランダムなプリセット
milkdrop-open-folder = プリセットのフォルダを開く
milkdrop-rescan = 再スキャン
milkdrop-rotation = ローテーション
    .description = 次へ、前へ、時間切り替えがめぐるプリセットの範囲
milkdrop-rotation-all = すべてのプリセット
milkdrop-empty = プリセットが見つかりません。
    .description = 下のフォルダーにパックを入れて、再スキャンを押してください。おすすめはこの3つ、どれも GitHub の projectM-visualizer にあります。
milkdrop-packs-hint = プリセットパックはここに置く。おすすめは presets-cream-of-the-crop、presets-milkdrop-original、presets-milkdrop-texture-pack の 3 つで、いずれも GitHub の projectM-visualizer にある。
milkdrop-no-gl = Milkdrop を起動できなかった。
milkdrop-still-starting = Milkdrop はまだ起動中。
milkdrop-still-starting-detail = グラフィックドライバーから OpenGL コンテキストを待っている。
milkdrop-no-frames = Milkdrop は動作中だが、フレームが届いていない。
milkdrop-no-frames-detail = { $renderer }、{ $version } で描画中。詳細はコンソールに。
milkdrop-favorite = プリセットをお気に入りに
milkdrop-favorite-short = お気に入り
milkdrop-favorited = お気に入り済み
milkdrop-favorites-only = お気に入りのみ
milkdrop-rotation-favorites = お気に入り ({ $count })
milkdrop-no-favorites = まだ星を付けたものがないので、ライブラリ全体をシャッフルします。リスト、パネルメニュー、または表示中の行からプリセットに星を付けてください。
milkdrop-reveal = プリセットの場所を開く
milkdrop-reveal-short = 場所を開く
milkdrop-current = 表示中
milkdrop-no-preset = まだ何も読み込まれていない
milkdrop-color = 色
    .description = プリセットはファイル本来の色のまま。テーマはライトテーマのとき明るさを反転する。パレットはテーマの背景色とアクセント色で塗り、アルバムの色は使わない。ジャケットは再生中のジャケットの色で塗る。曲ごとのテーマの設定に関わらず
milkdrop-color-preset = プリセット
milkdrop-color-theme = テーマ
milkdrop-color-palette = パレット
milkdrop-color-cover = ジャケット
milkdrop-name-always = 名前を常に表示
    .description = 名前を数秒ではなく常に表示し続ける
milkdrop-flip-horizontal = 左右反転
    .description = ビジュアルを左右に反転する
milkdrop-flip-vertical = 上下反転
    .description = ビジュアルを上下に反転する
milkdrop-choose = プリセットを選択
milkdrop-picker-title = プリセットを選択
milkdrop-picker-backdrop = 背景
milkdrop-picker-host = { $host } のプリセットを選択中
milkdrop-view-tree = ツリー
milkdrop-view-grid = グリッド
milkdrop-nested = フォルダー別
milkdrop-locate-current = 再生中へ移動
milkdrop-load-preset = プリセットを読み込む
milkdrop-unfavorite = お気に入りから外す
