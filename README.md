...
| Area      | What's there                                                                                                                                                                                                        |
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Library   | Parallel scanner reading full tags, true durations, and each file's own codec, sample rate, and bit depth, live folder watching that handles renames, files with unreadable tags indexed by filename so nothing silently drops, folder tree, filters, and search, cue sheets whose tracks become real rows instead of one long file, a quick-play box over the whole workspace on one key, a duplicate finder that groups copies and trashes the ones you don't keep, ffmpeg-backed conversion writing into a folder pattern you name, sort names read from the files or filled in from MusicBrainz, romanized readings for Japanese, Korean, and Chinese names, and a Library Health window of tiles that open the tracks missing each tag or the pass that fixes them |
| Playback  | Gapless single-stream engine, queue with shuffle, repeat, and play-next, endless play that refills the queue from your browse order, from what you've neglected, or from what the current track sounds like, crossfade that leaves album-contiguous boundaries alone, recovery when an audio device disappears, media keys and now-playing integration on all three platforms, A-B repeat with no gap at the splice, a sleep timer that lets the current track finish, and bookmarks you drop with a key, shown as chevrons on the seek strip and waveform and listed in their own panel |
...
| Audio     | A ten-band equalizer in a window of its own, ReplayGain off the tags (Opus R128 gains included) with an EBU R128 pass for the files no tagger ever measured, Opus decoded in pure Rust, windowed-sinc resampling, and an exclusive output mode (ALSA, WASAPI, CoreAudio) that reports the format the hardware accepted |
| Panels    | Forty-odd panel types (library, queue, history, bookmarks, playlists, lyrics, cover, biography, artist and genre grids, spectrum, spectrogram, oscilloscope, waveform, VU, milkdrop, particles), composed freely, duplicated with independent configs, popped out into OS windows, saved as named presets, drawers that slide out over a panel and a mini layout you shrink into, and a design mode switch so a finished layout stays put |
...
| Milkdrop  | MilkDrop presets through libprojectM, in a panel or as a backdrop behind every window over the blurred cover art, with a preset browser (thumbnails, favourites, rotation over the library, one folder, or your favourites, and a lock), colour modes that keep the preset's palette, flip it for the light theme, paint it in the theme's, or tint it from the playing cover, and a render thread that stops when nothing plays |
| Tagging   | Full tag editor with atomic writes, batch edits, and find and replace over a batch (literal or regex, previewed with a count), multi-value genres with merges kept as a library opinion rather than a tag write, ratings stored in the files themselves (FMPS and POPM), online tag and cover lookup through MusicBrainz, iTunes, Deezer, and TheAudioDB, artist biographies, and a pass that embeds what rox stores (lyrics, gains, acoustic descriptions) into the files so another player reads it too |
...
| History   | A full listen log driving the history panel and stats window, Last.fm, ListenBrainz, and Libre.fm scrobbling at one shared threshold, with Last.fm favourites and loved tracks synced in both directions                                                                          |
| Playlists | Favourites, drag reorder across playlists, M3U, PLS, and XSPF import and export, smart playlists that stay a query with their own sort and limit, entries that persist when their files leave and come back                            |
